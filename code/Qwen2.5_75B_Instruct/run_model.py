"""
데이터를 외부에서 불러오지 않고 임의로 설정한 query를 inference 코드
(line 244에서 query 설정)
...
def run_generation(
    ...
    prompts = ["대한민국 헌법 1조 1항에 대해 설명해줘", "방화구획의 설치기준 중 스프링클러 기타 이와 유사한 자동식 소화설비를 설치한 10층 이하의 층은 몇 m2 이내마다 구획하여야 하는가?"] * (batch_size // dist.get_world_size())
    # prompts = ["대한민국 헌법 1조 1항에 대해 설명해줘", "방화구획의 설치기준 중 스프링클러 기타 이와 유사한 자동식 소화설비를 설치한 10층 이하의 층은 몇 m2 이내마다 구획하여야 하는가?"]
